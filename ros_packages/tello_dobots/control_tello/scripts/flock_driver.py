...
from control_tello_msgs.msg import Flip, FlightData
import av
import cv2
import numpy
import tellopy
...
class FlockDriver(object):
    ...
    def __init__(self):
        ...
        self.pub_caminfo = rospy.Publisher('camera/camera_info', CameraInfo, queue_size=10, latch= True)
    ...
    def video_worker(self):
        ...
        for frame in container.decode(video=0):
            ...
            image_raw = self._cv_bridge.cv2_to_imgmsg(color_mat, 'bgr8')
            image_raw.header.frame_id = self.camera_frame			
            # Convert OpenCV Mat => ROS Image message and publish
            self._image_pub.publish(image_raw)
